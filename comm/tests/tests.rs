...
fn slice_arr_to_str(arr: &[&str]) -> String {
...
fn check_from_str(first: &[&str], second: &[&str], expected_output: &[&str]) {
    let str1 = slice_arr_to_str(first);
    let str2 = slice_arr_to_str(second);
    let second_content = FileContent::from(&str1);
    let mut output = FileContent::from(
        &str2
    ).common_lines(&second_content);
    output.sort();
...
#[test]
fn it_works_from_str() {
    check_from_str(&["foo"], &["foo"], &["foo"]);
    check_from_str(&["foo", "bar"], &["bar", "baz"], &["bar"]);
    check_from_str(
        &["apple", "orange", "potato"],
        &["pear", "orange", "banana"],
        &["orange"],
